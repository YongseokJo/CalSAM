...
################################## Read System Argv ##################################
    ...
    para_path      = sys.argv[2]      ### path to the actual run
    ...
    z0snap         = sys.argv[6]      ### redshift 0 snapshot
    tau_star_0     = sys.argv[7]
    epsilon_SN_0   = sys.argv[8]
    alpha_rh       = sys.argv[9]
    f_eject_thresh = sys.argv[10]
    YIELD          = sys.argv[11]
    f_Edd_radio    = sys.argv[12]
    f_return       = sys.argv[13]
    res_path       = sys.argv[14]
...
0\n\
'%(res_path, lib_path, tau_star_0, epsilon_SN_0, alpha_rh, f_eject_thresh,
   YIELD, f_Edd_radio, f_return, res_dict[sim_name], res_path, snaplen, txt)

################################## parameter file content ##################################


with open('%s/%s'%(para_path,para_name), "w") as f:
    f.write(content)
...
with open('%s/files.list'%(res_path), "w") as f:
    f.write(content)
